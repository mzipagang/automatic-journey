...
class TestAsyncHttpClientService(IsolatedAsyncioTestCase):
    __mock_external_api_http_client_service: MagicMock
    __subject_external_api_client: AsyncHttpClientService
    ...
    def setUp(self):
        ...
        self.__mock_external_api_http_client_service.get = AsyncMock(return_value=Response(
            status_code = 200, content = self.__test_token
        ))
        self.mock_http_connection_service.get_http_client().get = AsyncMock()
        self.mock_http_connection_service.get_http_client().post = AsyncMock()
        self.mock_http_connection_service.get_http_client().put = AsyncMock()
        self.mock_http_connection_service.get_http_client().patch = AsyncMock()
        self.mock_http_connection_service.get_http_client().aclose = AsyncMock()
        ...
        self.__subject_external_api_client = AsyncExternalApiHttpClientService(config_service=self.mock_config_service,
                                                                               http_connection_service=self.mock_http_connection_service)

        self.subject_koddi_client = AsyncKoddiHttpClientService(
            config_service=self.mock_config_service,
            http_connection_service=self.mock_http_connection_service,
            external_api_http_client_service=self.__mock_external_api_http_client_service)

    async def test_internal_get__should_succeed(self):
        await self.__subject_external_api_client.get('/path')
    ...
    async def test_internal_post__should_succeed(self):
        await self.__subject_external_api_client.post('/path', json={'key': 'value'})
    ...
    async def test_internal_put__should_succeed(self):
        await self.__subject_external_api_client.put('/path', json={'key': 'value'})
    ...
    async def test_internal_patch__should_succeed(self):
        await self.__subject_external_api_client.patch('/path', json={'key': 'value'})
    ...
    async def test_internal_close__should_succeed(self):
        await self.__subject_external_api_client.close()

        self.mock_http_connection_service.get_http_client().aclose.assert_called_once()


    async def test_koddi_post__should_succeed(self):
        await self.subject_koddi_client.post('/path', json={'key': 'value'})
    ...
    async def test_get__returns_response_if_response_requires_json(self):
        response = Response(status_code=200, json={'key': 'value'})
        self.mock_http_connection_service.get_http_client().get = AsyncMock(return_value=response)

        result = await self.__subject_external_api_client.get(path='/path', response_body_type='json')

        self.assertEqual(response, result)

    async def test_get__returns_response_if_response_is_json_and_has_required_data_field(self):
        response = Response(status_code=200, json={'data': 'value'})
        self.mock_http_connection_service.get_http_client().get = AsyncMock(return_value=response)

        result = await self.__subject_external_api_client.get(path='/path', response_body_type='json', json_has_data_field=True)

        self.assertEqual(response, result)

    async def test_get__logs_and_raises_server_error_if_json_does_not_exist(self):
        ...
        exception = None
        try:
            await self.__subject_external_api_client.get(path='/path', response_body_type='json')
    ...
    async def test_get__logs_and_raises_server_error_if_required_data_does_not_exist(self):
        request = Request(url='https://example.com/test', method='GET')
        ...
        exception = None
        try:
            await self.__subject_external_api_client.get(path='/path', response_body_type='json', json_has_data_field=True)
        except HTTPException as e:
            exception = e
    ...
    async def test_get__logs_and_raises_server_error_if_response_has_5xx_status(self):
        request = Request(url='https://example.com/test', method='GET')
    ...
    async def test_get__logs_and_raises_client_error_if_response_has_4xx_status(self):
        ...
        exception = None
        try:
            await self.__subject_external_api_client.get(
                path='/path',
    ...
    async def test_get__logs_and_raises_client_error_if_response_has_4xx_status_and_json_body(self):
        expected_status_code = 422
        expected_content = {
    ...
    async def test_get__logs_and_raises_server_error_if_response_is_outside_of_normal_range(self):
    ...
    async def test_get__returns_response_if_body_type_is_none(self):
        request = Request(url='https://example.com/test', method='GET')
        response = Response(status_code=200, request=request)
        self.mock_http_connection_service.get_http_client().get = AsyncMock(return_value=response)

        result = await self.__subject_external_api_client.get(path='/path')

        self.assertEqual(response, result)

    async def test_get__returns_response_if_body_type_is_json(self):
        request = Request(url='https://example.com/test', method='GET')
        response = Response(status_code=200, request=request, json={'key': 'value'})
        self.mock_http_connection_service.get_http_client().get = AsyncMock(return_value=response)
